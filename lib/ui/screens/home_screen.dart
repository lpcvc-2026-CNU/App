...
class _HomeScreenState extends State<HomeScreen>
  ...
  void initState() {
    ...
    // 포그라운드 푸시 알림 수신 시 스낵바로 사용자에게 즉시 표시
    PushNotificationService.instance.messageStream.listen((message) {
      if (mounted) {
        final title = message.notification?.title ?? "푸시 알림";
    ...
    // FCM 푸시 토큰 디버그 출력
    final initialToken = PushNotificationService.instance.token;
    if (initialToken != null) {
      print("[FCM 디버그] 현재 기기 토큰: $initialToken");
    }
    PushNotificationService.instance.tokenStream.listen((token) {
      print("[FCM 디버그] 신규 갱신 토큰: $token");
    });
  }
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    border: Border.all(
                      color: const Color(0xFFE61E2B).withOpacity(0.7),
                      width: 2,
                    ),
                      ...
                      Positioned(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            boxShadow: [
                              BoxShadow(
                                color: const Color(0xFFE61E2B).withOpacity(0.8),
                                blurRadius: 10,
                                spreadRadius: 2,
                              ),
                            ],
